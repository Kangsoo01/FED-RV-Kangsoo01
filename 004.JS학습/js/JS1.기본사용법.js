...
function 김비서나와라() {
  ...
  // - var는 variable(변수)라는 단어에서 파생된 말이다
  ...
  // 만약 속성명을 잘못 작성하면 브라우저가 출력에서 제외한다
  // 예전 브라우저는 에러가 발생했지만, 요즘에는 에러가 안난다
  ...
  // 나바디.style.backgroundColor = "linear-gradient(45deg, lightgreen, lightblue)";
  나바디.style.transition = "3s 5s";

  /******************************
     [ JS DOM의 요소 선택 메서드 ]
      1. id 선택 메서드
      = getElementById(아이디명) -> document.getElementById(아이디명)

      2. class 선택 메서드
      = getElementsByClassName(클래스명) -> document.getElementsByClassName(클래스명)
      ((하위속성))
      length = 클래스개수
      ((순번선택 하위메서드))
      ...
      3. tag 선택 메서드
      = getElementsByTagname(태그명) -> document.getElementsByTagname(태그명)
      ((하위속성))
      length = 클래스개수
      ((순번선택 하위메서드))
      item(순번) = 구체적인 순번요소 선택(0부터 시작)
...
function 맘대로해라(헐, 헉스) {
  ...
  /*********************** 
   [ JS DOM의 특별한 선택 메서드 2가지 ]
    ...
    1. querySelectorAll(CSS선택자) = 여러개 선택
    -> 2번방식은 HTMLCollection을 반환한다
    -> 따라서 lenght와 item(순번)/[순번]을 사용한다
   ***********************/
  ...
  아파트.style.right =
    "calc(100% - 200px*"+(헐+1)+")";
  // 순번에 1을 더해서 곱한 이유는 right 위치를 순서대로 잡기위함이다.
  ...
  // 3-4. iframe 디자인 넣기: 선택된 아파트 하위 iframe
  var 뮤비 = 아파트.querySelector('iframe');
  // cssText 속성 = 한꺼번에 문자열로 CSS를 넣을 때 사용한다
  // 주의: 따로 속성 셋팅할 때와 달리 다른 인라인 속성을 덮어버려서 지워버리니 조심해야한다
  뮤비.style.cssText = `
  position: absolute;
  border: none;
  width: 100%;
