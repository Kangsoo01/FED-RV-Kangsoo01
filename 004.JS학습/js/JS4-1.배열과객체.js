...
let choiceBtn = target[6].querySelectorAll("button");

// 버튼 개수 구하기
let cntBtn = choiceBtn.length;

console.log("초이스버튼들:", choiceBtn, cntBtn, "개");
...
for (let i = 0; i < cntBtn; i++) {
  ...
  choiceBtn[i].onclick = function () {
    ...
    // 버튼에 클래스 on 모두 빼기
    for (let i = 0; i < cntBtn; i++) {
      choiceBtn[i].classList.remove("on");
    } ////// for문 ///
    // 클릭된 버튼에 클래스 on넣기
    // 클릭된 버튼 자신은 this!
