...
// undefined는 if문에서 false 처리된다
function goR1() {
  // console.log("토끼자동이동!", autoI);

  // 인터발 변수에 할당하여 멈출 수 있게한다
  // 이때 변수 할당 전에는 변수가 undefind 이므로
  // if문으로 한번 할당 후엔 실행을 못하게 막아준다
  if (!autoI) {
    // 할당 전 false일 때, !(Not연산자)로 true로 변경
    // 인터발 호출
    autoI = setInterval(() => {
      // 1) 토끼 위치값 변수
      r1pos++;
    ...
    console.log("레벨옵션 값:", level.value);
  } // if end
} ///////// goR1함수 //////////////////
...
function whoWinner() {
  ...
  // -> 토끼 인터벌 멈추기 + 거북 클릭작동 막기
  if (r1pos >= FINAL_NUM || t1pos >= FINAL_NUM) {
    // (1) 토끼 멈추기
    clearInterval(autoI);

    // (2) 거북이 멈추기(거북멈춤 상태로 변경)
    t1Stop = true;

    // 승자판별 함수 (메세지: 토끼 / 거북 / 비김)
    let winner;
    ...
    // (4) 선택된 객체의 배열값 개수로 랜덤 수 만들기
    let rdmNum = Math.floor(
      Math.random() * msgTxt[winner].length
    );
