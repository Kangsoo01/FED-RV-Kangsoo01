...
// 음식 배열값이 있는 경우 출력하기 /////
ReactDOM.render(
  <WishList wList={foods} gubun="음식" />,
  root[2]
);

// 음식 배열값이 없는 경우 출력하기 /////
ReactDOM.render(
  <WishList wList={[]} gubun="음식" />,
  root[3]
);
...
ReactDOM.render(
  <WishList wList={movs} gubun="영화" />,
  root[4]
);

// 영화 위시리스트 출력하기 : 배열값이 없는 경우 ///
ReactDOM.render(
  <WishList wList={[]} gubun="영화" />,
  root[5]
);
...
function ShowLikePainter({ isChange }) {
  ...
  return (
    <React.Fragment>
      ...
      {/* 2. 변경버튼 */}
      {/* 
      클릭 시 상태관리변수를 업데이트하여 상태변수를 
      사용하고 있는 곳이 변경되게 만든다
      !result 는 true 값이면 false, false 값이면 true
      */}
      <button
        onClick={() => {
          setResult(!result);
          console.log("변경 후 result 값:", result);
        }}
        style={{
          fontSize: "30px",
          padding: "10px",
      ...
      {/* 3. 작품출력 */}
      {
        ...
        // -> 상태관리변수 result를 사용하여
        // result 값이 변경될 때 컴포넌트가 갱신되도록 한다
...
// 4-3. 컴포넌트 호출하기 ////////
ReactDOM.render(
  <ShowLikePainter isChange={false} />,
  root[6]
);
